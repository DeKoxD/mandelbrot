document.addEventListener('DOMContentLoaded', event => {
  ...
  var draw = loadCanvasFractal(canvas);
...
function loadCanvasFractal(canvas) {
  var img;
  var extra = 1.5;
  return async function(refetch) {
    var { offsetx, offsety } = canvas.dataset;
    var { centerx, centery, zoom, resx, resy } = canvas.dataset;
    [resxextra, resyextra] = [Math.ceil(resx*extra), Math.ceil(resy*extra)];
    var ctx = canvas.getContext('2d');
    if (refetch || !img) {
      var raw = await fetchFractal(centerx, centery, zoom, resxextra, resyextra, true, false);
